...
pub mod open_rails {
    ...
    pub fn create_organizer(ctx: Context<CreateOrganizer>, name: String) -> Result<()> {
        require!(name.as_bytes().len() <= MAX_NAME, OpenRailsError::NameTooLong);
    ...
    pub fn create_action_type(
        ...
        require!(code.as_bytes().len() <= MAX_ACTION_CODE, OpenRailsError::CodeTooLong);
        require!(name.as_bytes().len() <= MAX_NAME, OpenRailsError::NameTooLong);
        require!(uri.as_bytes().len() <= MAX_URI, OpenRailsError::UriTooLong);
    ...
    pub fn submit_claim(
        ...
        require!(occurred_ts <= now + 3600, OpenRailsError::OccurredTimeInFuture);

        if let Some(c) = &context {
            require!(c.as_bytes().len() <= MAX_CONTEXT, OpenRailsError::ContextTooLong);
        }
        ...
        match claim_kind {
            ClaimKind::AttendedEvent => {
                let event = ctx.accounts.event.as_ref().ok_or(OpenRailsError::MissingEvent)?;

                // must be within event window
                require!(
                    occurred_ts >= event.start_ts && occurred_ts <= event.end_ts,
                    OpenRailsError::OutsideEventWindow
                );

                // claim code must match (QR/link token)
                let provided = event_claim_code.ok_or(OpenRailsError::MissingClaimCode)?;
                require!(provided == event.claim_code, OpenRailsError::InvalidClaimCode);

                claim.event = event.key();
                claim.action_type = Pubkey::default();

                // POA doesn't require evidence in MVP
            ...
            ClaimKind::RealWorldAction => {
                ...
                if action_type.requires_evidence {
                    require!(evidence_hash.is_some(), OpenRailsError::EvidenceRequired);
                }

                claim.event = Pubkey::default();
                claim.action_type = action_type.key();
    ...
    pub fn approve_claim(ctx: Context<ApproveClaim>) -> Result<()> {
        ...
        let claim = &mut ctx.accounts.claim;
        require!(claim.status == ClaimStatus::Pending, OpenRailsError::ClaimNotPending);
    ...
    pub fn attach_badge(
        ...
        require!(badge_uri.as_bytes().len() <= MAX_URI, OpenRailsError::UriTooLong);
...
pub struct SubmitClaim<'info> {
    ...
    /// Optional: Event for POA
    pub event: Option<Account<'info, Event>>,

    /// Optional: ActionType for PRA
    pub action_type: Option<Account<'info, ActionType>>,

    #[account(
        init,
        payer = claimant,
        space = 8 + Claim::INIT_SPACE,
        seeds = [b"claim", organizer.key().as_ref(), claimant.key().as_ref(), claim_id.key().as_ref()],
        bump
    )]
...
pub struct ApproveClaim<'info> {
    ...
    #[account(
        ...
        seeds = [b"proof", claim.organizer.as_ref(), claim.claimant.as_ref(), claim.key().as_ref()],
        bump
    )]
